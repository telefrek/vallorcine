...
for index_file in .kb/CLAUDE.md .decisions/CLAUDE.md; do
    ...
    local_content="$(cat "$index_file" 2>/dev/null)" || continue

    local_rows="$(count_data_rows "$local_content")"
    ...
    elif [[ "$main_content" != "$local_content" ]]; then
        # Same row count but different content — entries may have been updated
        # Only warn if main has lines not present locally
        main_only="$(diff <(echo "$local_content") <(echo "$main_content") 2>/dev/null | grep '^>' | grep -c '^> |' || echo "0")"
