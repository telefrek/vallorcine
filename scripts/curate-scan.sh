...
if [[ -s "$TMPDIR_SCAN/spec-graduation.txt" ]]; then
    echo "## Active Deprecations (status: DEPRECATED + state: APPROVED)" >> "$SUMMARY_FILE"
    echo "Specs that have been marked deprecated and are still load-bearing." >> "$SUMMARY_FILE"
    echo "Per \`rules/deprecation-discipline.md\`, each must declare its mode:" >> "$SUMMARY_FILE"
    echo "" >> "$SUMMARY_FILE"
    echo "- **Succession mode** — non-empty \`displaced_by\` pointing at the" >> "$SUMMARY_FILE"
    echo "  alternative spec. Migrate consumers before \`removal_scheduled_in\`." >> "$SUMMARY_FILE"
    echo "- **Retirement mode** — \`retirement: true\` (no successor; code" >> "$SUMMARY_FILE"
    echo "  path going away entirely). Retire dependent code before removal." >> "$SUMMARY_FILE"
    echo "" >> "$SUMMARY_FILE"
    echo "When the work lands, transition to \`state: INVALIDATED\` — the" >> "$SUMMARY_FILE"
    echo "validator enforces the audit-trail-before-deletion contract" >> "$SUMMARY_FILE"
    echo "(displacement_reason + reproducer + \`.kb/_legacy/<spec-id>.md\`)." >> "$SUMMARY_FILE"
    echo "" >> "$SUMMARY_FILE"
    echo "Deprecation is a **one-way ratchet** — there is no rescind to" >> "$SUMMARY_FILE"
    echo "ACTIVE/STABLE. If the alternative turns out insufficient, amend" >> "$SUMMARY_FILE"
    echo "the alternative or write a third spec; do not un-deprecate." >> "$SUMMARY_FILE"
    echo "" >> "$SUMMARY_FILE"
