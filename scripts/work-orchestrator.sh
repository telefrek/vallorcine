...
state_write() {
  local dir="$1"
  local group_slug started_at last_tick_at epoch cap paused
  ...
  # Defaults — guard against missing/corrupt fields producing invalid JSON.
  # CRITICAL: numeric fields MUST validate as integers before interpolation,
  # or state.json ends up with literal `"epoch": ,` (invalid). Same for cap.
  [[ -z "$paused" ]] && paused="false"
  [[ ! "$epoch" =~ ^-?[0-9]+$ ]] && epoch=0
  [[ ! "$cap" =~ ^[1-9][0-9]*$ ]] && cap=3
  [[ -z "$group_slug" ]] && group_slug="$(basename "$(dirname "$dir")")"
  [[ -z "$started_at" ]] && started_at="$(iso_now)"
  # paused is a JSON literal (true/false), not a quoted string — accept only those.
  [[ "$paused" != "true" && "$paused" != "false" ]] && paused="false"

  # Note: pause_reason was removed from state.json after the 2026-05-11
  # adversarial review (state_get truncated at the first `"` inside the
  # value, corrupting state.json on round-trip when an escalation question
  # contained quoted symbols). The blocked/ directory IS the source of
  # truth — each blocked/<wd>.json carries the per-WD reason. status
  # enumerates blocked/ to show "paused because:". No regex round-trip.
...
read_specified_set() {
  ...
  local readiness="$group_dir/_readiness.json"

  # HIGH 5 (2026-05-11 adversarial): verify the group directory exists
  # BEFORE running work-resolve. Otherwise a typo'd group slug silently
  # produces an empty queue.
  if [[ ! -d "$group_dir" ]]; then
    echo "ERROR: work group directory does not exist: $group_dir" >&2
    return 1
  fi

  # Trigger regeneration. work-resolve writes _readiness.json as a side
  # effect. Propagate its exit code — silent swallowing would let init
  # produce a misleadingly-empty queue on resolver failure.
  local rc
  (cd "$(dirname "$work_root")" && bash "$SCRIPT_DIR/work-resolve.sh" "$group" >/dev/null 2>&1)
  rc=$?
  if [[ $rc -ne 0 ]]; then
    echo "ERROR: work-resolve.sh exited $rc for group '$group'" >&2
    return 1
  fi
  ...
  # HIGH 6 (2026-05-11 adversarial): prefer jq if available — the awk
  # parser below is coupled to work-resolve.sh's hard-coded 4-space
  # indentation, which is undocumented. jq is robust to any valid JSON
  # shape. Fall back to awk with a stderr warning when jq is missing.
  if command -v jq >/dev/null 2>&1; then
    jq -r '.wds[] | select(.status == "SPECIFIED") | .id' "$readiness"
    return 0
  fi

  echo "WARN: jq not found — falling back to awk parser (coupled to work-resolve indentation)" >&2
  awk '
    /^    \{/ { in_wd = 1; id = ""; status = "" }
    in_wd && /"id":/ {
...
cmd_init() {
  ...
  # HIGH 5 (2026-05-11 adversarial): validate readiness BEFORE committing
  # to the orchestrator directory. If work-resolve fails (typo'd group,
  # missing fixture, resolver error), rollback so re-init isn't blocked
  # by a half-built directory.
  local readiness_output
  if ! readiness_output=$(read_specified_set "$group" 2>&1 >/dev/null && read_specified_set "$group"); then
    echo "ERROR: cannot initialize orchestrator — readiness check failed" >&2
  ...
  # Populate initial queue from SPECIFIED set. (readiness_output captured
  # above so we don't re-run work-resolve.)
  local wd added=0
...
cmd_status() {
  ...
  local epoch cap paused started last_tick
  ...
  if [[ "$paused" == "true" ]]; then
    echo "Paused:     YES — escalation(s) awaiting user input:"
    for f in "$dir/blocked"/*.json; do
      [[ -e "$f" ]] || continue
      local bwd brs
      bwd=$(basename "$f" .json)
      brs=$(grep -oE '"reason":[[:space:]]*"[^"]*"' "$f" | sed -E 's/.*"([^"]*)"/\1/')
      echo "              $bwd — $brs"
    done
  else
    echo "Paused:     no"
...
cmd_dispatch() {
  ...
  # MEDIUM 8 (2026-05-11 adversarial): an empty feature_slug propagated
  # silently into in-flight/<wd>.json and broke hung-detection's path
  # lookup. Require non-empty.
  if [[ -z "$feature_slug" ]]; then
    echo "ERROR: feature_slug must be non-empty for dispatch" >&2
    exit 2
  fi
  ...
  # MEDIUM 9 (2026-05-11 adversarial): order the guards so the error
  # message tells the truth. "already in-flight" is more informative
  # than "not in queue" for a double-dispatch.
...
cmd_complete() {
  ...
  # not re-queue it. The reverse order (write then rm) would leave the
  # WD in BOTH sets — unrecoverable without operator intervention.
...
cmd_resume() {
  ...
  # Adversarial finding (2026-05-11): a single resume cleared paused even
  # when other WDs remained blocked, defeating the design contract that
  # "paused=true while ANY WD awaits user input". Require an empty
  # blocked/ set before resuming.
  local blocked_count
  blocked_count=$(find "$dir/blocked" -name '*.json' 2>/dev/null | wc -l | tr -d ' ')
  if [[ "$blocked_count" -gt 0 ]]; then
    echo "ERROR: $blocked_count WD(s) still blocked — unblock all before resuming" >&2
    for f in "$dir/blocked"/*.json; do
      [[ -e "$f" ]] || continue
...
cmd_hung() {
  ...
  for f in "$dir/in-flight"/*.json; do
    ...
    if [[ ! -f "$status_md" ]]; then
      ...
      # GNU date supports -d; BSD date (macOS) supports -j -f.
      mtime=$(date -u -d "$disp" +%s 2>/dev/null \
            || date -u -j -f "%Y-%m-%dT%H:%M:%SZ" "$disp" +%s 2>/dev/null \
            || echo "")
    else
      # GNU stat: -c %Y. BSD stat (macOS): -f %m.
      mtime=$(stat -c %Y "$status_md" 2>/dev/null \
            || stat -f %m "$status_md" 2>/dev/null \
            || echo "")
    fi

    # HIGH 4 (2026-05-11 adversarial): if both stat/date variants fail we
    # MUST NOT silently treat the WD as fresh — that's exactly the failure
    # mode hung was built to catch. Treat unknown-mtime as epoch 0 so
    # delta is enormous and the WD is flagged. Warn to stderr so the
    # operator knows the platform check failed.
    if [[ -z "$mtime" || ! "$mtime" =~ ^[0-9]+$ ]]; then
      echo "WARN: hung: could not determine mtime for $wd ($feat) — treating as hung" >&2
      mtime=0
    fi
