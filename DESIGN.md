...
│   ├── work-orchestrator.sh        ← persistent state machine for /work-run dynamic-DAG dispatch (init/status/dump/ready/dispatch/complete/block/unblock/skip/resume/hung/resolve/clear); state lives at .work/<group>/.orchestrator/ with queue.txt + in-flight/ + completed/ + blocked/ subset directories so transitions are atomic mv operations; no jq dependency
